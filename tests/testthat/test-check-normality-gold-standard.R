...
test_that("tyler_check_normality - normal data returns list with mean and sd", {
  df <- data.frame(value = NORMAL_DATA)
  result <- suppressMessages(tyler_check_normality(df, "value"))
...
test_that("tyler_check_normality - normal data mean is correct gold-standard value", {
  df <- data.frame(value = NORMAL_DATA)
  result <- suppressMessages(tyler_check_normality(df, "value"))
...
test_that("tyler_check_normality - skewed data returns list with median and iqr", {
...
test_that("tyler_check_normality - skewed data median = 1.5 (gold standard)", {
  df <- data.frame(value = SKEWED_DATA)
  result <- suppressMessages(tyler_check_normality(df, "value"))
  expect_equal(result$median, 1.5, tolerance = 1e-10)
})

test_that("tyler_check_normality - skewed data IQR = 8.4 (gold standard)", {
  df <- data.frame(value = SKEWED_DATA)
  result <- suppressMessages(tyler_check_normality(df, "value"))
  expect_equal(result$iqr, 8.4, tolerance = 1e-10)
})

test_that("tyler_check_normality - missing variable name errors with informative message", {
  df <- data.frame(x = 1:10)
  expect_error(
    suppressMessages(tyler_check_normality(df, "nonexistent")),
    "Variable not found"
  )
})

test_that("tyler_check_normality - sample size < 3 errors", {
  df <- data.frame(v = c(1, 2))
  expect_error(
    suppressMessages(tyler_check_normality(df, "v")),
    "at least 3"
  )
})

test_that("tyler_check_normality - NAs are stripped before test (n≥3 after stripping)", {
  df <- data.frame(v = c(NA, 1, 2, NA, 3, 4, 5))
  # Five non-NA values: 1,2,3,4,5 → small dataset, test should run without error
  expect_no_error(suppressMessages(tyler_check_normality(df, "v")))
})

test_that("tyler_check_normality - all-NA except 2 errors on sample size", {
...
test_that("tyler_check_normality - returns a named list (not NULL)", {
  df <- data.frame(val = NORMAL_DATA)
  result <- suppressMessages(tyler_check_normality(df, "val"))
  expect_false(is.null(result))
  expect_true(length(result) > 0)
})

test_that("tyler_check_normality - result for normal data has numeric mean and sd", {
  df <- data.frame(val = NORMAL_DATA)
  result <- suppressMessages(tyler_check_normality(df, "val"))
...
test_that("tyler_check_normality - result for skewed data has numeric median and iqr", {
...
test_that("tyler_check_normality - constant vector: sd = 0, mean = constant (passes Shapiro)", {
  ...
  tryCatch(
    {
      result <- suppressWarnings(suppressMessages(tyler_check_normality(df, "v")))
...
test_that("tyler_check_normality - large-n normal data (n=5000) returns mean/sd", {
  set.seed(1)
