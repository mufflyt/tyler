...
test_that("tyler_table_percentages - single clear winner A=4/8=50%", {
  df <- data.frame(category = c("A", "B", "A", "C", "A", "B", "B", "A"))
  # A=4, B=3, C=1, total=8; only A is the max
  result <- tyler_table_percentages(df, "category")
...
test_that("tyler_table_percentages - three-way tie returns all three", {
  # A=3, B=3, C=3
  df <- data.frame(x = c("A", "B", "A", "B", "C", "C", "C", "A", "B"))
  result <- tyler_table_percentages(df, "x")
...
test_that("tyler_table_percentages - single-level vector is 100%", {
  df <- data.frame(v = rep("Only", 20))
  result <- tyler_table_percentages(df, "v")
  expect_equal(nrow(result), 1L)
...
test_that("tyler_table_percentages - NAs are counted as their own level by dplyr::count", {
  df <- data.frame(cat = c("A", NA, "A", "C", "A", "B", "B", NA))
  # Without na.rm: A=3, B=2, NA=2, C=1, total=8; A wins
  result <- tyler_table_percentages(df, "cat")
  expect_equal(result$n[1], 3L)
  expect_equal(result$percent[1], 37.5)
})

test_that("tyler_table_percentages - variable passed as string is accepted", {
  df <- data.frame(state = c("CO", "CO", "CA", "NY"))
  result <- tyler_table_percentages(df, "state")
  expect_equal(result$state[1], "CO")
  expect_equal(result$n[1], 2L)
  expect_equal(result$percent[1], 50)
})

test_that("tyler_table_percentages - uses actual taxonomy Classification column", {
  e <- new.env()
  data("taxonomy", package = "tyler", envir = e)
  result <- tyler_table_percentages(e$taxonomy, "Classification")
...
test_that("tyler_table_percentages - two items tied with 50% each", {
  df <- data.frame(ins = c("Medicaid", "Private", "Medicaid", "Private"))
  result <- tyler_table_percentages(df, "ins")
  expect_equal(nrow(result), 2L)
  expect_true(all(result$n == 2L))
  expect_true(all(result$percent == 50))
})

test_that("tyler_table_percentages - result columns are always n and percent", {
...
test_that("tyler_table_percentages - percents sum to 100 across full distribution", {
...
test_that("tyler_table_proportion - equal split Male/Female 50/50", {
  df <- data.frame(gender = c("Male", "Female", "Female", "Male", "Male", "Female"))
  result <- tyler_table_proportion(df, gender)
  expect_s3_class(result, "data.frame")
  expect_equal(nrow(result), 2L)
  expect_true(all(result$n == 3L))
  expect_true(all(result$percent == 50))
...
test_that("tyler_table_proportion - percents sum to 100", {
  df <- data.frame(grade = c("A", "B", "A", "C", "B", "A", "C", "B"))
  result <- tyler_table_proportion(df, grade)
  expect_equal(sum(result$percent), 100, tolerance = 0.01)
})

test_that("tyler_table_proportion - three levels gold standard", {
  # Medicaid=4, Private=3, Self-pay=3, total=10
  df <- data.frame(ins = c(
    rep("Medicaid", 4), rep("Private", 3), rep("Self-pay", 3)
  ))
  result <- tyler_table_proportion(df, ins)
  expect_equal(nrow(result), 3L)
...
test_that("tyler_table_proportion - single row is 100%", {
  df <- data.frame(x = rep("solo", 7))
  result <- tyler_table_proportion(df, x)
  expect_equal(nrow(result), 1L)
  expect_equal(result$n[1], 7L)
  expect_equal(result$percent[1], 100)
})

test_that("tyler_table_proportion - n column is integer counts not fractions", {
  df <- data.frame(cat = c("X", "X", "Y"))
  result <- tyler_table_proportion(df, cat)
  expect_true(all(result$n == floor(result$n)))
})

test_that("tyler_table_proportion - percent is numeric between 0 and 100", {
...
test_that("tyler_table_proportion - returns all levels, not just majority", {
