...
# tyler_most_common_gender returns a single character string.
...
test_that("tyler_most_common_gender - gold standard sentence structure", {
  result <- tyler_most_common_gender(SIMPLE_DATA)
  expect_type(result, "character")
  expect_length(result, 1L)
})

test_that("tyler_most_common_gender - contains gender 'male' at 60%", {
  result <- tyler_most_common_gender(SIMPLE_DATA)
  expect_true(grepl("male", result, ignore.case = FALSE))
  expect_true(grepl("60", result))
})

test_that("tyler_most_common_gender - contains specialty 'Cardiology' at 60%", {
  result <- tyler_most_common_gender(SIMPLE_DATA)
  expect_true(grepl("Cardiology", result))
})

test_that("tyler_most_common_gender - contains credential 'MD' at 60%", {
  result <- tyler_most_common_gender(SIMPLE_DATA)
  expect_true(grepl("MD", result))
})

test_that("tyler_most_common_gender - academic affiliation is lowercased in output", {
  result <- tyler_most_common_gender(SIMPLE_DATA)
  # Function calls tolower() on academic_affiliation value
  expect_true(grepl("yes", result, ignore.case = FALSE))
})

test_that("tyler_most_common_gender - 70/30 split gender gold standard", {
  df <- data.frame(
...
test_that("tyler_most_common_gender - sentence contains four percentages", {
  result <- tyler_most_common_gender(SIMPLE_DATA)
...
test_that("tyler_most_common_gender - all NAs in gender column yields NaN%", {
  df <- data.frame(
    gender                   = c(NA, NA, NA),
    specialty                = c("X", "X", "Y"),
...
test_that("tyler_most_common_gender - ties broken by alphabetical order", {
  df <- data.frame(
...
test_that("tyler_most_common_gender - result ends with a period", {
  result <- tyler_most_common_gender(SIMPLE_DATA)
  expect_true(endsWith(result, "."))
})

test_that("tyler_most_common_gender - proportion rounds to one decimal", {
  df <- data.frame(
...
test_that("tyler_most_common_gender - missing required column errors", {
  df <- data.frame(
  ...
  expect_error(tyler_most_common_gender(df))
})

test_that("tyler_most_common_gender - purely female dataset says female in output", {
  df <- data.frame(
