...
test_that("input validation works correctly", {
  expect_error(plot_and_save_emmeans("not a model", "cyl", "cyl", "cyl", tempdir()),
               "model_object must be a valid model class object")
  expect_error(plot_and_save_emmeans(test_model, list("cyl"), "cyl", "cyl", tempdir()),
               "specs must be a single character string")
  expect_error(plot_and_save_emmeans(test_model, "cyl", 123, "cyl", tempdir()),
               "variable_of_interest must be a single character string")
  expect_error(plot_and_save_emmeans(test_model, "cyl", "cyl", "cyl", "nonexistent/directory"),
               "Output directory does not exist")
})

# Test for correct output structure
test_that("function returns correct output structure", {
  result <- plot_and_save_emmeans(test_model, "cyl", "cyl", "cyl", tempdir())
...
test_that("function creates a plot file", {
  output_dir <- tempdir()
  plot_and_save_emmeans(test_model, "cyl", "cyl", "cyl", output_dir)
...
test_that("function handles emmeans errors correctly", {
  ...
  expect_error(plot_and_save_emmeans(bad_model, "cyl", "cyl", "cyl", tempdir()), "Error in calculating emmeans")
})
