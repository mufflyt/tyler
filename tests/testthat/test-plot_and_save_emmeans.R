...
test_that("input validation works correctly", {
  expect_error(plot_and_save_emmeans("not a model", "cyl", "cyl", output_dir = tempdir()),
               "model_object must be a valid model class object")
  expect_error(plot_and_save_emmeans(test_model, list("cyl"), "cyl", output_dir = tempdir()),
               "specs must be a single character string")
  expect_error(plot_and_save_emmeans(test_model, "cyl", 123, output_dir = tempdir()),
               "variable_of_interest must be a single character string")
  expect_error(plot_and_save_emmeans(test_model, "cyl", "cyl", output_dir = "nonexistent/directory"),
               "Output directory does not exist")
})

# Test for correct output structure
test_that("function returns correct output structure", {
  result <- plot_and_save_emmeans(test_model, "cyl", "cyl", output_dir = tempdir())
...
test_that("function creates a plot file", {
  ...
  plot_and_save_emmeans(test_model, "cyl", "cyl", output_dir = output_dir)
...
test_that("function handles emmeans errors correctly with nonexistent spec", {
  expect_error(
    plot_and_save_emmeans(test_model, "nonexistent_variable", "nonexistent_variable", output_dir = tempdir()),
