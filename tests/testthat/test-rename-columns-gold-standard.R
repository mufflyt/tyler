...
test_that("tyler_rename_columns - basic rename preserves other columns", {
  df <- data.frame(doctor_info = 1:5, patient_data = 6:10)
  result <- suppressMessages(
    tyler_rename_columns(df, c("doctor_info"), c("physician_info"))
  )
  expect_equal(names(result), c("physician_info", "patient_data"))
  expect_equal(result$physician_info, 1:5)
})

test_that("tyler_rename_columns - data values are unchanged after rename", {
  df <- data.frame(old_name = c(10, 20, 30))
  result <- suppressMessages(
    tyler_rename_columns(df, c("old_name"), c("new_name"))
  )
  expect_equal(result$new_name, c(10, 20, 30))
})

test_that("tyler_rename_columns - case-insensitive matching (UPPER → lower target)", {
...
test_that("tyler_rename_columns - multiple renames in one call", {
...
test_that("tyler_rename_columns - mismatched vector lengths error", {
  df <- data.frame(a = 1, b = 2)
  expect_error(
    tyler_rename_columns(df, c("a", "b"), c("x")),
    "same length"
  )
})

test_that("tyler_rename_columns - non-matching target emits warning, not error", {
  df <- data.frame(existing = 1:3)
  expect_warning(
    suppressMessages(
      tyler_rename_columns(df, c("no_such_col"), c("new_name"))
    ),
    "nothing was renamed"
  )
})

test_that("tyler_rename_columns - rename log attribute is attached to result", {
  df <- data.frame(col_x = 1:4)
  result <- suppressMessages(
    tyler_rename_columns(df, c("col_x"), c("col_y"))
  )
...
test_that("tyler_rename_columns - row count is unchanged", {
  df <- data.frame(foo = 1:100, bar = 101:200)
  result <- suppressMessages(
    tyler_rename_columns(df, c("foo"), c("baz"))
  )
  expect_equal(nrow(result), 100L)
})

test_that("tyler_rename_columns - no-op (empty vectors) returns data unchanged", {
  df <- data.frame(a = 1:3, b = 4:6)
  # mismatched-length guard fires before loop, so empty-empty is actually valid
  result <- suppressMessages(
    tyler_rename_columns(df, character(0), character(0))
  )
...
test_that("tyler_clean_phase2 - accepts a data frame directly", {
...
test_that("tyler_clean_phase2 - renames columns to standard_names", {
...
test_that("tyler_clean_phase2 - writes a CSV file to output_directory", {
...
test_that("tyler_clean_phase2 - output_path attribute is set on successful run", {
...
test_that("tyler_clean_phase2 - empty required_strings stops with error", {
  df <- data.frame(a = 1)
  expect_error(
    suppressMessages(
      tyler_clean_phase2(df, required_strings = character(0), standard_names = character(0))
    )
  )
})

test_that("tyler_clean_phase2 - non-existent file path stops with error", {
  expect_error(
    suppressMessages(
      tyler_clean_phase2(
        "/no/such/file.csv",
...
test_that("tyler_clean_phase2 - clean_names lowercases column names before matching", {
...
test_that("tyler_clean_phase2 - non-data-frame non-path input stops with error", {
  expect_error(
    suppressMessages(tyler_clean_phase2(42L, c("a"), c("b"))),
