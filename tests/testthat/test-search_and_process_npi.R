...
test_that("Processes data frame input correctly", {
  with_mocked_bindings(
    ...
    .package = "npi",
    code = {
      result <- mysterycall_search_and_process_npi(sample_data, notify = FALSE)
...
test_that("Handles empty input data frame", {
  ...
  result <- mysterycall_search_and_process_npi(empty_data, notify = FALSE)
  expect_equal(nrow(result), 0)
})

test_that("validates required columns", {
  expect_error(mysterycall_search_and_process_npi(data.frame(first = "A"), notify = FALSE), "last")
})
...
test_that("Handles invalid NPIs gracefully", {
  with_mocked_bindings(
    ...
    code = {
      ...
      result <- mysterycall_search_and_process_npi(invalid_data, notify = FALSE)
      expect_equal(nrow(result), 0)
    }
  )
})
...
test_that("Accumulates results and resumes processing", {
  ...
  with_mocked_bindings(
    ...
    .package = "npi",
    code = {
      result_1 <- mysterycall_search_and_process_npi(
        data,
      ...
      result_2 <- mysterycall_search_and_process_npi(
        data[1, , drop = FALSE],
        accumulate_path = temp_accumulate,
        resume = TRUE,
        notify = FALSE
  ...
  with_mocked_bindings(
    ...
    code = {
      for (tc in test_cases) {
        ...
        result <- mysterycall_search_and_process_npi(data, notify = FALSE)
        if (tc$first == "Invalid") {
          expect_equal(nrow(result), 0)
        } else {
...
test_that("Handles large datasets efficiently", {
  ...
  with_mocked_bindings(
    ...
    code = {
      start_time <- Sys.time()
      result <- mysterycall_search_and_process_npi(large_data, notify = FALSE)
