...
create_scatter_plot <- function(plot_data,
  ...
  checkmate::assert_numeric(plot_data[[y_var]], any.missing = TRUE, lower = 0, finite = TRUE)
  checkmate::assert_true(is.factor(plot_data[[x_var]]) || is.character(plot_data[[x_var]]), .var.name = "x_var column")
  checkmate::assert_true(nrow(plot_data) > 0, .var.name = "filtered plot_data rows")
