...
clean_phase_1_results <- function(file_path) {
  ...
  library(stringr)
    ...
    df <- df %>%
      filter(!is.na(npi))  # Remove rows with missing 'npi'
    ...
    df <- df %>%
      mutate(for_redcap = paste(id, dr_name, insurance, phone_number, state_name, npi, academic, id_number, sep = ", ")) %>%
      select(for_redcap, id, phone_number, academic, everything())
