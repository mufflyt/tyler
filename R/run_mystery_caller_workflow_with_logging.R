...
run_mystery_caller_workflow_with_logging <- function(
  ...
  checkmate::assert_true(length(phase2_required_strings) == length(phase2_standard_names), .var.name = "phase2_required_strings")
  checkmate::assert_true(all(nzchar(trimws(lab_assistant_names))), .var.name = "lab_assistant_names")
  checkmate::assert_true(all(nzchar(trimws(split_insurance_order))), .var.name = "split_insurance_order")
  checkmate::assert_true(all(nzchar(trimws(phase2_required_strings))), .var.name = "phase2_required_strings")
  checkmate::assert_true(all(nzchar(trimws(phase2_standard_names))), .var.name = "phase2_standard_names")
  checkmate::assert_true(!is.null(names(npi_search_args)) || length(npi_search_args) == 0L, .var.name = "npi_search_args")
  checkmate::assert_true(nchar(trimws(output_directory)) > 0, .var.name = "output_directory")
  checkmate::assert_true(nchar(trimws(phase2_output_directory)) > 0, .var.name = "phase2_output_directory")
  checkmate::assert_true(nchar(trimws(phase1_output_directory)) > 0, .var.name = "phase1_output_directory")
  checkmate::assert_true(nchar(trimws(quality_check_path)) > 0, .var.name = "quality_check_path")
  checkmate::assert_true(is.null(all_states) || length(unique(all_states)) == length(all_states), .var.name = "all_states")
  checkmate::assert_true(is.null(taxonomy_terms) || all(nzchar(trimws(taxonomy_terms))), .var.name = "taxonomy_terms")
