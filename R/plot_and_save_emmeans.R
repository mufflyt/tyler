...
#' @details
#' The function computes estimated marginal means via [emmeans::emmeans()] with
#' `type = "response"`, builds a point-and-interval plot using `rate`,
#' `asymp.LCL`, and `asymp.UCL`, then writes a timestamped PNG file to
#' `output_dir`.
#'
#' It is intended for quick post-model summaries when you need both the EMM
#' table and a reusable ggplot object in one call.
#'
#' @import emmeans ggplot2 dplyr
#' @seealso [create_and_plot_interaction()], [check_normality()], [create_scatter_plot()]
