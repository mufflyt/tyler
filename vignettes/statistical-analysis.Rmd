...
```{r wait-time-dist, echo=TRUE, eval=TRUE, fig.width=6.5, fig.height=3.8, fig.cap="Simulated wait-time distributions by insurance type. Medicaid and Uninsured callers show longer right tails, consistent with Poisson count data."}
set.seed(42)
n_per <- 200L
...
```{r irr-forest, echo=TRUE, eval=TRUE, fig.width=5.5, fig.height=3.2, fig.cap="Forest plot of incidence rate ratios (IRR) from the Poisson GLMM. Error bars are 95 % profile-likelihood confidence intervals. The vertical dashed line at IRR = 1 represents no difference from the Private insurance reference group."}
irr_df <- data.frame(
