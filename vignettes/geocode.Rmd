...
---
# Overview
The `geocode` function is designed to help you geocode datasets containing addresses and change them to latitude and longitude.  
...
`mysterycall_geocode()` accepts a CSV file path. Under the hood it calls the
Google Maps Geocoding API via `ggmap::geocode()` — the same service AHRQ uses
for practice-location geocoding. You will need a Google Maps API key with the
Geocoding API enabled; set it in your `.Renviron` file:

```
GOOGLE_MAPS_API_KEY=AIza...
```

**Deduplicate addresses before geocoding.** If 300 providers share 150 unique
practice addresses, geocode the 150 unique addresses and join back — this halves
...
The output CSV adds `lon` and `lat` columns (decimal degrees, WGS84) plus a
`geocode_status` column (`"OK"`, `"ZERO_RESULTS"`, or `"REQUEST_DENIED"`).
Always inspect the status column before proceeding:
...
The geocoded result has one row per unique address. Rejoin to the full provider
roster with `mysterycall_safe_left_join()` so every provider row gets
coordinates, and so any address-matching failures are visible rather than silent.
...
Some downstream functions (e.g. `create_isochrones_for_dataframe()`) expect
separate numeric `lat` and `long` columns rather than an `sf` geometry column.
If you are working with an `sf` object, extract coordinates with:
...
**Do not geocode from ZIP codes alone.** ZIP-code centroids place practices at
the post-office, not the clinic — a meaningful spatial error that compounds in
the isochrone step.

# Conclusion

`mysterycall_geocode()` converts raw practice addresses into latitude/longitude
coordinates suitable for mapping, drive-time modeling, and spatial joins.
Combine it with `mysterycall_search_by_taxonomy()` to build a fully enriched
provider roster — from NPI retrieval through geocoded locations — in a single
reproducible pipeline. Next, pass the geocoded file into
`create_isochrones_for_dataframe()` to build drive-time polygons
(see `vignette("create_isochrones", package = "mysterycall")`).

# Features and bugs
If you have ideas for other features that would make geocoding easier, or find a
bug, the best approach is to report it at the package issue tracker.
