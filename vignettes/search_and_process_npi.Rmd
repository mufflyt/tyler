...
# Remove punctuation and special characters from the 'basic_credential' column to clean "M.D., MD, M D"
processed_result <- processed_result %>%
  dplyr::mutate(basic_credential = stringr::str_remove_all(basic_credential, "[[\\p{P}][\\p{S}]]"))

# Convert 'basic_credential' to uppercase
processed_result <- processed_result %>%
  dplyr::mutate(basic_credential = stringr::str_to_upper(basic_credential))

# Remove whitespace characters from the 'basic_credential' column
processed_result <- processed_result %>%
  dplyr::mutate(basic_credential = stringr::str_remove_all(basic_credential, "[:blank:]"))

# Recode values in the 'basic_credential' column, mapping "DOMPH" to "DO" and defaulting to "MD"
processed_result <- processed_result %>%
  dplyr::mutate(basic_credential = dplyr::recode(basic_credential, "DOMPH" = "DO", "DO" = "DO", .default = "MD"))

# Filter out rows where 'taxonomies_state' is NA (missing)
processed_result <- processed_result %>%
  dplyr::filter(!is.na(taxonomies_state))
...
# Keep only distinct rows based on the 'npi' column, retaining the first occurrence
processed_result <- processed_result %>%
