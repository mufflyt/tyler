...
This checklist is tailored to the current repository and is intended to make CRAN submission reproducible.

## 1) Preflight metadata checks

- [ ] Confirm `DESCRIPTION` fields are final for release (especially `Version`, `Date`, `Title`, `Description`, `URL`, `BugReports`).
...
Top-level analysis/ops markdown files should not ship in the CRAN tarball.
